...
- [x] One typed provider module exports exactly these constants and no aliases:
...
- [x] `exchange.api.v1` is bound to Service Account bearer authentication and the exact two-route
...
- [x] `exchange.effective-catalogue-response.v1` is bound directly to
...
- [x] `exchange.invoke-request.v1` is the existing raw operation JSON body — no envelope — at the
...
- [x] `exchange.invoke-response.v1` is bound to the exact success
...
- [x] Provider fixtures live under `tests/fixtures/exchange-http-v1/` with a checked filename/SHA-256
      ...
      fixture fails first.
- [x] A future compatible additive/change policy is written before implementation. Any wire change
...
- 2026-08-04: Delivered one typed protocol source, strict production request/response decoders and
  a checked provider fixture inventory covering the two authenticated Milestone 1 HTTP routes and
  their complete bounded refusal set.
