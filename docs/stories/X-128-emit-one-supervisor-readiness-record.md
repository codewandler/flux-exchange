...
- [x] The server has one documented supervised launch mode distinct from ordinary interactive and
...
- [x] The inherited-handle discovery ABI is exact. Unix runs only `--supervised`: readiness is the
...
- [x] After every required store and safety check succeeds and the socket has bound, Exchange writes
...
- [x] The process identity contains both the PID as a diagnostic and an OS-derived start identity
...
- [x] The release/build and protocol portion is produced from the same typed source as
      ...
      opens no store.
- [x] The one-shot channel carries no log line, progress event, HTTP byte or later control traffic.
...
- [x] A separate inherited liveness pipe makes Exchange cooperate with owner death on every target.
...
- [x] No credential, setting, grant body, Service Account token/verifier, session value or control
      ...
      them.
- [x] **Failing-first process tests:** a successful child emits one record only after bind; a store
...
- [x] Native Linux, macOS and Windows tests spawn the real server through the exact inherited ABI,
