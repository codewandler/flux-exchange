...
**v0.0.1 — a charter and a type system.** `crates/exchange-host` carries the vocabulary and the
rules as tested types (19 tests); `crates/exchange-server` prints a deployment matrix and exits;
`console/` renders fixtures. **There is no service yet.** See [README](../../README.md) § Status.
