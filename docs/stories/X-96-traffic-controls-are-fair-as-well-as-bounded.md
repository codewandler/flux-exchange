...
## Acceptance
- [ ] Preserve the process-wide sign-in, invocation-rate and invocation-concurrency limits as the
      final application backstop; saturation still refuses immediately with `429` and
      `Retry-After`, without an unbounded queue.
- [ ] Add per-principal invocation budgets after identity resolution and before vendor dispatch.
      Tenant/principal keys come only from the resolved principal, never a caller header or body.
- [ ] Put anonymous flood controls at the trusted edge for sign-in and other anonymous endpoints.
...
- [ ] Expose bounded metrics for admitted/refused work, saturation and active invocations, labelled
      without principal identifiers or attacker-controlled high-cardinality values.
- [ ] Failing-first tests prove one principal cannot exhaust another's budget, the global ceiling
...
- [ ] Alert on sustained anonymous and authenticated saturation without logging tokens or request
      bodies.
