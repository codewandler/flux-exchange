...
- [ ] Failing first, a hermetic self-test proves a free-form test transcript can grow without bound
      and does not bind the exact story, commit, check profile or terminal outcome.
- [ ] A repository script accepts an explicit story id and closed targeted-check profile, refuses a
      dirty or mismatched checkout before execution, and emits one versioned JSON receipt.
- [ ] The receipt carries commit, profile, exact commands, outcomes and durations under a hard byte
...
- [ ] Unknown profiles, timeout, signal termination and command failure remain typed terminal
...
- [ ] Hermetic self-tests cover pass, fail, timeout, oversized output, unknown profile and dirty
      checkout behavior without network, release keys or provider credentials.
- [ ] Contributor guidance describes this as targeted story evidence only; the complete Cargo,
