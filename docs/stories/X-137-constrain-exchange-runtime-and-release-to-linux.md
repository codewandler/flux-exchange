...
- [x] The Exchange release target authority contains exactly
...
- [x] The production server/helper native boundary is Linux-only: `getpwuid_r(geteuid())` derives
...
- [x] macOS support roots and `getpeereid`, plus every Windows profile/ACL, named-pipe,
...
- [x] X-137 contracts the supported-target policy, `release-targets.tsv`, distribution target set,
...
- [x] Any mechanical fixture regeneration needed to keep the repository gate green is explicitly an
...
- [x] The X-134, X-138, X-139 and X-126 contracts and the linked design are committed on canonical
...
- [x] Changes are limited to Exchange-owned runtime/release paths. The published portable
...
- [x] Targeted Linux tests cover owner root/peer refusal, supervised readiness/liveness, helper
...
- 2026-08-05: The complete repository gate passed. The authority-selected x86_64 runner executed
