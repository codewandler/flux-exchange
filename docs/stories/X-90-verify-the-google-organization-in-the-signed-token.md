...
## Acceptance
- [ ] Add an expected hosted-domain deployment setting. When set, `/api/signin` sends `hd` only as a
      Google account-selection hint; the hint is never treated as admission.
- [ ] Carry the signed ID token's `hd` claim across the verified-claims seam and require byte-for-byte
      equality with the configured value. A missing or mismatched claim refuses with no session.
- [ ] Never derive membership from `email` or an email suffix. Identity remains keyed by immutable
      OIDC `sub`.
- [ ] Reduce the sign-in scope to `openid`; remove unused `email`/`profile` claims and fields unless
      a concrete in-tree consumer is identified in the same change.
- [ ] Failing-first tests cover a matching, missing and mismatched signed claim; a mutation that
      trusts the authorization-request hint or email turns the gate red.
- [ ] Update the deployment configuration and runbook without committing an organization identifier
