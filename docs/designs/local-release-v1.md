...
`fixture-set.json.exchange_commit` names the committed provider/verifier baseline from which the
set was generated; it is deliberately the parent of the commit that refreshes this manifest, since
a file cannot contain the Git hash of the commit that contains that same file. The separately
recorded fixture-set SHA-256 identifies the exact final manifest bytes. `native_cases` maps each
platform-dependent verdict to its closed target set, Cargo test target and exact test name. The
five native release jobs first verify that mapping and then list and execute each named test with
`--exact`; a broad suite invocation or a zero-test filter is not evidence for a fixture verdict.
