...
**Lock 2 — one seam, counted.** A scanner over `crates/exchange-host/src/**/*.rs` enforces eleven
...
| `connector_pack::Rehearsal` | `settings.rs` | the pack's **third** entry point turning up somewhere new. It takes no `Egress`, holds no transport and has no `execute`, so nothing reached through it can dispatch — this is a count, not a refusal, and the point of the count is that `resolve` and `pack` were once believed to be the whole list | a *fifth* entry point. The row below is the fourth, and it is the demonstration that this column was right: it shipped upstream in `connector-pack` 0.23, was adopted in `settings.rs` by X-152, and was invisible here until somebody counted it. The rule knows the four that exist |
| `connector_pack::DocumentRehearsal` | `settings.rs` | the pack's **fourth** entry point — the document-backed twin of the row above, answering the same questions from the catalogue's canonical document instead of from a runtime parse of the operation's emitted Flux. Counted for the same reason and on the same terms: no `Egress`, no transport, no `execute`, so this is a count rather than a refusal. What sharpens it is that `DocumentRehearsal::request` composes a request through the same `connector_resolve::build_request` the live path calls, so a second file entering it is a second file deriving a request | the same call under an alias or a re-export, and — as with every row here — a *fifth* entry point nobody has read the changelog for. It also says nothing about *how many times* `settings.rs` enters it: `tests/no_connector_flux_parse.rs` is what holds the count to exactly one file, and holds it in the other direction too, since a crate that stopped naming it at all would satisfy this row |
